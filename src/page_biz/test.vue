<template>
    <div class="fillcontain">
        ...
        <div class="table_container" style="padding-bottom: 0">
            ...
            <el-button style="float: right" type="primary" @click="goto_edit_test(0)">新增测试</el-button>
        ...
        <div class="table_container">
            <el-table
                ...
                <el-table-column label="操作" width="300">
                    <template slot-scope="scope">
                        <el-button size="mini" @click="goto_edit_test(scope.row.id)">编辑</el-button>
...
<script>
    ...
    import {test_list,test_del,test_verify,test_sort} from '@/api/getDatatest'
    export default {
        data(){
            return {
                tableData: [],
...
//                categories:[],
        ...
        methods: {
            list() {
                test_list({page:this.currentPage,page_size:this.limit,title:this.title}).then(function(res){
            ...
            goto_edit_test(id) {
                this.$router.push({path:'add_test',query:{id:id}});
            },
            verify(scope, status) {
                ...
                this.$confirm('确认此操作?', '提示', {
                    ...
                    test_verify({id:item.id,status:status}).then(function(res){
                        if (res.code == this.$store.state.constant.status_success) {
                            item.status = status;
                            this.$message({
                                type: 'success',
            ...
            del(item, index) {
                ...
                this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
                    ...
                    test_del({id:item.id}).then(function(res){
                        if (res.code == this.$store.state.constant.status_success) {
