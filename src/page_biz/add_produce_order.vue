        ...
        beforeRouteUpdate(to, from, next) {
            ...
            if (this.active == 1 && this.way == 1) {
                ...
                if (!cache_sales_order_selection) {
                    ...
                    this.$router.replace({path:'add_produce_order',query:{active:0,way:1}});
                }
            ...
            get_sales_sub_order_list(){
                ...
                sales_sub_orders({page:this.currentPage,page_size:this.limit,status:1}).then(function(res){
                    if (res.code == this.$store.state.constant.status_success) {
                        ...
                        setTimeout(function(){
                            this.toggleSelection(this.multipleSelection['page'+this.currentPage]);
                        }.bind(this))
...
//                };
                ...
                for (var i in this.multipleSelection) {
                    ...
                    if (this.multipleSelection[i] && this.multipleSelection[i].length) {
                        this.multipleSelection[i].forEach(function(val, index){
                            ...
                            if (sales_order_selection[unique][color_unique].order_sub_ids.indexOf(val.id) == -1) {
                                sales_order_selection[unique][color_unique].order_sub_ids.push(val.id);
                                sales_order_selection[unique][color_unique].order_sub.push({sub_id:val.id,order_id:val.order_id});//用于提交
                            }
                            if (sales_order_selection[unique][color_unique].order_codes.indexOf(val.sales_order.order_no) == -1) {
                ...
                for (i in sales_order_selection) {
                    ...
                    for(var j in sales_order_selection[i]) {
                        if (block.product_id) {
                            block.colors.push({color:{id:sales_order_selection[i][j].data.color_id,name:sales_order_selection[i][j].data.color_code},
                ...
                this.$router.push({path:'add_produce_order',query:{active:1,way:1}});
            },
                ...
                for (var i in this.multipleSelection) {
                    if (this.multipleSelection[i] && this.multipleSelection[i].length) {
                        this.multipleSelection[i].forEach(function(val){
                            if (!check) {
                                check = val.product_cat_id + '|' + val.product_id + '|' + val.color_id + '|' + val.color_code;
                            } else {
                                var current_check = val.product_cat_id + '|' + val.product_id + '|' + val.color_id + '|' + val.color_code;
...
//                    this.$refs.multipleTable.clearSelection();
            ...
            submit: function () {
                ...
                this.$confirm(tip, '提示', {
                    ...
                    this.loading = true;
                    produce_order_edit(data).then(function (res) {
                        if (res.code == this.$store.state.constant.status_success) {
                            this.$message({
                                message: res.msg,
                                type: 'success'
